...
# --- 统一工作流和动态 LoRA 配置 ---
BASE_WORKFLOW_FILENAME = "chmr-0522-base.json" # 用于所有任务的单个工作流文件
LORA_NODE_IDS = ["12"] # 工作流中 "Lora Loader Stack" 节点的 ID 列表
...
if __name__ == "__main__":
    ...
    # --- 按迭代轮次重新组织任务 ---
    tasks_by_iteration = {}
    for iter_num, scene_folder_name, shot_folder_name, image_filename, scene_num_str in tasks_to_run:
        if iter_num not in tasks_by_iteration:
            tasks_by_iteration[iter_num] = []
        tasks_by_iteration[iter_num].append((scene_folder_name, shot_folder_name, image_filename, scene_num_str))

    print(f"\n--- 开始按轮次执行处理 ({NUM_ITERATIONS} 轮迭代) ---")

    # --- 对每轮迭代分别执行 ---
    for current_iteration in range(1, NUM_ITERATIONS + 1):
        current_tasks = tasks_by_iteration.get(current_iteration, [])
        total_tasks_this_iteration = len(current_tasks)
        
        if total_tasks_this_iteration == 0:
            print(f"迭代轮次 {current_iteration}: 没有任务需要处理。")
            continue
        
        print(f"\n=== 开始执行第 {current_iteration} 轮迭代 (共 {total_tasks_this_iteration} 个任务) ===")
        
        futures_map = {}  # 重置每轮的futures映射
        
        # --- 线程池并发执行当前轮次的任务 ---
        with ThreadPoolExecutor(max_workers=NUM_WORKERS) as executor:
            # --- 提交当前轮次的所有任务 ---
            for i, (scene_folder_name, shot_folder_name, image_filename, scene_num_str) in enumerate(current_tasks):
                ...
                context_log = f"任务 {i+1}/{total_tasks_this_iteration} Srv{server_id_for_log} It{current_iteration} Sc{scene_num_str}-Sh{shot_folder_name}"
                ...
                future = executor.submit(
                    ...
                    current_iteration_num=current_iteration,
            ...
            # --- 等待当前轮次的所有任务完成并收集结果 ---
            tqdm.write(f"\n所有第 {current_iteration} 轮的 {total_tasks_this_iteration} 个任务已提交。等待此轮完成...\n")

            for future in tqdm(as_completed(futures_map),
                            total=total_tasks_this_iteration,
                            desc=f"第{current_iteration}轮处理", unit="个任务",
        ...
        print(f"\n=== 第 {current_iteration} 轮迭代完成 ===")
        print(f"迭代 {current_iteration} 完成统计: 成功 {tasks_succeeded_count}/{tasks_completed_count} 任务")
